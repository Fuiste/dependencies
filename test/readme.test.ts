...
describe('README examples', () => {
  it('quick start example works', async () => {
    ...
    const result = await use(compose(config, greeter), (context) =>
      Context.get(context, Greeter).greet(),
    )
  ...
  it('composition example works', async () => {
    ...
    const built = await build(
      compose(config, compose(logger, compose(database, app))),
    )
  ...
  it('scoped resources example works', async () => {
    const Database = Context.Tag<{ query: () => string }>(
      'readme/scope/database',
    )
    const events: string[] = []
  ...
  it('testing and override example works', async () => {
    const Repository = Context.Tag<{ source: string }>(
      'readme/override/repository',
    )
